...
## 2026-07-12 추가수정 성공
### 작업 내용
- 자정 넘김 리다이렉트 범위를 arrive 이벤트까지 확장, 새벽 2시 컷오프 추가 (`scripts/log_location.py`)

### 결과
- 성공: depart류만 감지하던 자정 넘김 리다이렉트에 출근성(arrive, location != home) 이벤트도 추가. 전날 근무가 안 닫힌 상태에서 창고 도착 같은 arrive 이벤트가 자정 넘겨 발생하면 오늘 파일에 잘못 새 time_in이 잡히던 문제(사장님이 오늘 수동 정정) 해결. 단, 새벽 2시 컷오프를 둬서 2시 이후는 자동 리다이렉트 없이 정상적으로 새 날짜에 기록되도록 함. commute_in은 frontmatter를 안 건드리는 이벤트라 대상에서 제외. same-day 처리 로직은 무수정.

### 검증
- 로컬 격리 테스트 7케이스(정상 당일 / 퇴근 리다이렉트 2시 이전 / 출근 리다이렉트 2시 이전 신규 / 2시 컷오프 신규 depart·arrive / home 이벤트는 리다이렉트 제외 / 전날 이미 닫힘 / 전날 파일 없음) 모두 통과, py_compile 통과
