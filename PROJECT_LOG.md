...
## 2026-07-30 성공 (아이폰 음성입력 연동 — URL 파라미터 자동전송)
### 작업 내용
- 조사: iOS Safari는 Web Speech API(SpeechRecognition) 자체가 미지원(WebKit 엔진 한계, Apple 개발자 포럼에서 버그로 확인) — 앱 안에 마이크 버튼을 넣는 방식은 아이폰에서 불가능함을 실제 웹 조사로 확인
- 대안 조사: iOS 단축어(Shortcuts)의 "Get Contents of URL" 액션으로 GitHub Actions를 직접 트리거한 선례 확인 → "받아쓰기 → URL 열기" 조합이면 Siri 음성명령으로 완전히 가능
- chatbot.html에 `?q=문장&auto=1` URL 파라미터 처리 추가: q는 입력칸 자동채움, auto=1이 같이 있어야 자동전송. 인덱스 로딩 완료(buildIndex) 이후에 전송하도록 순서 보장. 새로고침 시 중복전송 방지를 위해 처리 즉시 history.replaceState로 URL 정리

### 결과
- 성공: 로컬 임시 Playwright 스크립트(저장소 미포함)로 두 케이스 검증 — (1) q+auto=1: 자동으로 사용자말풍선+장소기록 확인말풍선까지 표시, dispatch 페이로드(place+lat+lng) 정확, URL 파라미터 정리됨 (2) q만(auto 없음): 입력칸엔 채워지되 자동전송은 안 됨(user 말풍선 0개). 두 케이스 모두 페이지 에러 0건

### 배운 것 / 반복하면 안 되는 실수
- 클라이언트 사이드 API 가용성은 실제로 조사해서 확인하고 답할 것 — "아마 될 것"으로 추측 답변하지 않고 웹 조사·소스 링크로 확인 후 안내

### 성공 루틴 (재사용 가능한 방법)
- 플랫폼 제약(iOS 브라우저 API 미지원)에 부딪히면, 그 플랫폼의 다른 네이티브 경로(iOS 단축어)로 우회하되 기존에 검증된 로직(classifyIntent 등)을 그대로 재사용하는 방향으로 설계 — 로직 중복 구현 방지

## 2026-07-30 성공 (챗봇 UI 카카오톡 스타일 + 자연어 자동기록)
### 작업 내용
- chatbot.html 채팅 UI를 카카오톡 스타일로 변경(노란/흰 말풍선, 아바타, 시간표시) — 앞선 커밋에서 이미 반영
- 자연어 3갈래 자동 라우팅 추가: 문장 하나 입력하면 검색/장소방문/메모 중 자동 판별해서 처리
  - 검색: 물음표·의문사 있으면 기존 검색+AI 로직(변경 없음)
  - 장소방문: 저장된 장소명(2글자 이상)이 문장에 있고 질문이 아니면 자동 기록(index.html의 recordPlaceVisit과 동일하게 좌표 재사용)
  - 메모: 위 둘 다 아니면 dispatch({note, section:'memo'})로 기록(기존 메모팝업과 동일 백엔드 분류 재사용)
  - 신규 장소 등록·출퇴근·주유는 v1 범위 제외(기존 전용 화면 유지) — 사장 확인 후 확정한 범위
- index.html/chatbot.html 라벨을 "기록 검색"→"기록 챗봇"으로 통일(기능 확장 반영)

### 결과
- 성공: classifyIntent 순수 로직 단위테스트 9/9 통과(장소3/검색2/메모4, 오탐 방지 케이스 "집중해서 일함","창고지기가 됐어" 포함)
- 버그 발견·수정: 장소명 뒤 조사 판별 정규식에서 문자클래스([...])에 "에서","까지" 같은 여러 글자를 넣으면 개별 글자(예:'지')로 쪼개져 매칭되는 실수 — "창고지기가 됐어"가 "창고" 장소기록으로 오분류됨. 알드터네이션(`(에서|까지|...)`)으로 수정 후 재검증 통과
- 성공: 로컬 임시 Playwright 스크립트(저장소 미포함)로 dispatch() 호출을 가로채 실제 전송 바디까지 확인 — 장소기록 1회(place+lat+lng), 메모기록 1회(note+section), 검색은 dispatch 미호출(회귀 없음, 결과카드 2건) 모두 정상. 페이지 에러 0건

### 배운 것 / 반복하면 안 되는 실수
- 정규식 문자클래스 `[...]`는 "여러 글자로 된 단어들의 OR"가 아니라 "그 안의 개별 문자 중 하나"로 동작한다 — 여러 글자 조사/단어를 매칭하려면 반드시 알터네이션 `(a|b|c)`를 써야 함. 이번처럼 조용히 오탐을 만들 수 있어 반드시 케이스 테스트로 잡아야 함

### 성공 루틴 (재사용 가능한 방법)
- 새 자동분류 로직은 먼저 순수 함수로 분리해 Node 단위테스트로 빠르게 검증(브라우저 없이), 이후 Playwright로 실제 dispatch 바디까지 통합 검증하는 2단계가 효율적
- 기존 화면의 dispatch/recordPlaceVisit 패턴을 그대로 재사용해 백엔드 계약(log_location.py의 NOTE/SECTION/PLACE 처리) 불일치 위험을 없앰
...
- 키 저장 시: 검색된 기록(최대 15건)을 시스템프롬프트 근거로 넣어 자연어 답변(callAI) + "참고한 기록" 목록 병기
- 키 없으면: 기존 키워드 검색만 그대로 동작(회귀 없음)

### 결과
- 성공: 실제 네트워크로 Gemini OpenAI호환 엔드포인트 CORS 프리플라이트 확인(access-control-allow-origin 허용) — 브라우저 직접 호출 가능함을 사전 검증
- 성공: 실제 오류 응답 형식이 배열([{error:{...}}])로 감싸져 오는 것을 fake key로 확인 → callAI가 배열/객체 두 형태 모두 파싱하도록 구현
- 성공: 로컬 임시 Playwright 스크립트(저장소 미포함, fetch 목킹)로 키없음(검색만)/AI성공(답변+참고기록)/AI배열에러(폴백+에러메시지 정상표시)/네트워크예외(폴백) 4가지 경로 모두 페이지 에러 0건 확인

### 배운 것 / 반복하면 안 되는 실수
- 외부 AI API를 정적사이트에서 직접 호출하기 전에 CORS 프리플라이트를 실제로 curl해 확인하는 습관 — 안 하면 배포 후에야 막히는 걸 알게 됨
- Gemini의 OpenAI 호환 레이어는 에러 응답이 표준과 다르게 배열로 옴 — 에러 파싱 시 두 형태 다 대비 필요

### 성공 루틴 (재사용 가능한 방법)
- 근거 없는 질문엔 AI를 안 부르고(검색결과 0건이면 스킵) 검색된 기록만 컨텍스트로 넘겨 환각 방지
- 키가 없어도 기존 기능이 100% 그대로 동작하게 만들어 하위호환 유지

### 배포 이력
- 버전: v2026.07.29.2 → v2026.07.29.3
- 배포 일시: 2026-07-30

## 2026-07-30 성공 (주간 자동요약 게시판 — 파트 A)
### 작업 내용
- data 브랜치에 scripts/weekly_summary.py 추가 — 지난 한 주(월~일) 일지+주유를 집계(AI 없음)해 weekly/YYYY-Www.md 생성
- .github/workflows/weekly-summary.yml 신설 — 매주 월요일 07:00 KST(cron '0 22 * * 0') 자동 실행 + workflow_dispatch 수동 재실행(ref_date 지정 가능)
- weekly.html 신설 — 게시판 목록(최신순) + 클릭 시 본문(자체 최소 마크다운 렌더러) 열람
- index.html에 "🗓 주간 요약" 카드 링크 추가, footer v2026.07.29.2

### 결과
- 성공: weekly_summary.py를 실제 data 브랜치 diary(2026-07-20~26)로 검증 — 근무일수/시간, 방문장소 횟수, 끼니, 메모, 주유 집계 정상
- 성공: 로컬 임시 Playwright 스크립트(저장소 미포함, fetch 목킹)로 목록 정렬/dir필터, 렌더링(h1/h2/li), <script> 태그 포함 악성 콘텐츠도 이스케이프되어 실행 안 됨(XSS 안전), 뒤로가기, 페이지 에러 0건 확인

### 배운 것 / 반복하면 안 되는 실수
- 앞선 세션에서 오늘 날짜(2026-07-29~30)를 커밋 시점 날짜(07-24)로 착각해 로그·버전에 오기입했던 실수가 있었음 — 이번엔 시스템 제공 날짜를 재확인 후 기록

### 성공 루틴 (재사용 가능한 방법)
- 자동화 스크립트는 scripts/log_location.py와 동일하게 data 브랜치에 직접 배치(git worktree로 로컬 체크아웃 후 커밋·푸시)
- 새 게시판형 페이지는 우리가 직접 생성하는 md 포맷이 단순하므로 외부 마크다운 라이브러리 대신 자체 최소 렌더러(escapeHtml 선행 후 패턴 치환)로 XSS 안전하게 구현

### 배포 이력
- 버전: v2026.07.29.1 → v2026.07.29.2
- 배포 일시: 2026-07-30

## 2026-07-29 성공 (기록 검색 챗봇)
### 작업 내용
- 새 페이지 chatbot.html — 내 자비스 기록(일지 본문 + 장소)에서 키워드로 검색하는 챗봇
- index.html에 "🔎 기록 검색" 카드/링크 추가, footer v2026.07.29.1
- (감사 반영) 시간 없는 불릿도 인덱싱하도록 extractLines 보강, 캐시키 v2로 갱신
- (자기점검) 날짜 오류 정정: 앞선 작업 로그/버전을 07-24로 잘못 표기 → 실제 오늘 2026-07-29 기준으로 수정

### 결과
- 성공: Git Trees API 1회로 diary 파일 전체 확보 → 각 md/places.json fetch → 클라이언트 인덱스 빌드(localStorage 캐시, tree sha로 갱신), 키워드 AND 검색 + 결과 없으면 OR 자동확장, <mark> 하이라이트, 결과 클릭 시 index.html?date=로 이동. 서버·외부 API키 불필요(기존 GitHub 토큰 재사용)
- 검증: 인라인 JS node --check + 로컬 임시 Playwright 스크립트(저장소 미포함, fetch 목킹) — 인덱스 10건 빌드/캐시, "신혜식당" 3건, 문장형 "당구 언제 쳤어" OR확장 2건, 하이라이트, 없음 케이스, 페이지 에러 0건 모두 통과

### 배운 것 / 반복하면 안 되는 실수
- 순수 AND 키워드 검색은 문장 질문("~ 언제 쳤어")에서 0건 → AND 실패 시 OR 폴백으로 보완 필요
- 한글 조사 붙은 토큰("신혜식당에")은 여전히 못 잡음(단순 includes 한계) — 단순 검색 범위로 수용

### 성공 루틴 (재사용 가능한 방법)
- 새 페이지는 vehicle.html 골격(head/테마4종/토큰배너/헤더/showResult) 복제 후 로직만 교체
- 전체 diary 순회는 contents 디렉터리 반복 대신 git/trees?recursive=1 1회가 효율적

### 배포 이력
- 버전: v2026.07.24.1 → v2026.07.29.1
- 배포 일시: 2026-07-29

## 2026-07-29 성공
### 작업 내용
- 장소 기록: 입력칸 클릭 시 저장된 장소 전체를 카테고리별로 정리해 드롭다운으로 표시
- 장소 이름 클릭 시 즉시 방문 기록(저장 좌표 사용)
- 각 항목에서 수정(이름+카테고리)/삭제 가능, GitHub places.json에 직접 커밋해 영구 반영

### 결과
- 성공: 포커스 시 전체 목록·그룹핑, 필터, 즉시기록, 수정(키 교체+좌표 보존), 삭제 모두 로컬 임시 Playwright 스크립트(저장소 미포함)로 검증 통과(에러 없음). base64 UTF-8 왕복 인코딩이 기존 읽기 로직과 대칭임을 확인

### 배운 것 / 반복하면 안 되는 실수
- 수정·삭제는 localStorage만 고치면 다음 places.json 동기화 때 되돌려짐 → Contents API PUT로 원본(data 브랜치) 반영 필수

### 성공 루틴 (재사용 가능한 방법)
- placeInput 드롭다운은 DOM createElement로 렌더(문자열 템플릿 대신) — 장소명 따옴표 이스케이프 버그 회피
- 저장은 savePlacesToGitHub()가 SHA 조회 후 PUT

### 배포 이력
- 버전: v2026.07.15.7 → v2026.07.24.1 (라이브 라벨; 실제 작업일 2026-07-29)
- 배포 일시: 2026-07-29
...
## 2026-07-22 (긴급) PC에서 앱 전체 백지 — Leaflet 렌더블로킹 → 지연 로딩으로 수정

### 작업 내용
- v2026.07.15.3 배포 후 사장님 PC(및 시크릿창)에서 **앱 전체가 백지**. 폰(다른 망)에서는 정상. 헤드리스 Chromium으로 로컬 로드 시 본문 정상 렌더(모든 카드, body visible, 텍스트 259자) → **코드/HTML/문법/배포 모두 정상** 확인. 즉 기기·네트워크 의존 문제로 좁힘.
- 원인 확정: OSM 원복 때 `<head>`에 다시 넣은 **Leaflet CSS `<link>`(unpkg.com)** 가 render-blocking 리소스인데, 사장님의 필터링된 PC 네트워크에서 unpkg 응답이 지연(hang)되면 브라우저가 그 CSS를 기다리며 화면 페인팅을 멈춰 **백지**가 됨. 카카오 버전엔 이 render-blocking `<head>` CSS가 없어 정상이었고, 폰은 unpkg가 정상 로드돼 정상. "지도 하기 전엔 잘됐어"라는 사장님 증언과 일치.

### 결과
- 성공: Leaflet(CSS+JS)을 `<head>`/스크립트에서 **완전 제거**하고, 지도 팝업 열 때만 동적 주입하는 `loadLeaflet()` 지연 로더 추가. `openMapPopup()`은 `loadLeaflet().then(...)`로 감싸 지도 초기화, 실패 시 토스트만. → 앱 본체는 unpkg에 의존/대기하지 않아 **네트워크와 무관하게 항상 즉시 렌더**. 지도는 열 때만 로드.
- 헤드리스 재검증(unpkg 차단 환경): 앱 본문 정상 렌더 + Leaflet 관련 콘솔 에러가 로드 시점에서 사라짐(4건→2건) 확인. `node --check` 통과. 폰에서 지도 표시·Nominatim 검색 정상 동작 사장님 확인 완료.

### 배운 것 / 반복하면 안 되는 실수
- 팝업 등에서만 쓰는 무거운/외부 라이브러리를 `<head>`에 render-blocking으로 올리지 말 것. 필터링·저속 네트워크에서 해당 CDN이 hang하면 **앱 전체가 백지**가 된다. 지도처럼 특정 화면에서만 쓰는 건 **지연 로딩(클릭 시 동적 주입)** 이 기본.
- "앱 전체 백지"는 헤드리스 브라우저로 실제 로드해 콘솔·body 상태를 직접 보면 코드 문제인지 네트워크 문제인지 즉시 갈린다 — 추측 대신 이 방법을 먼저 쓸 것.
...
## 2026-07-23 중복 기록 방지 (전송 중 재클릭 차단)

### 작업 내용
- `dispatch()`에 전송 중 중복 실행을 막는 가드 추가. 모든 기록(메모·장소·카테고리·오늘 정리·장소 방문)이 `dispatch()`를 거치므로 한 곳만 고쳐 전체 적용.
- 모듈 전역 `let _dispatching = false;` 플래그. `dispatch` 진입 시 이미 전송 중이면 "이미 전송 중이에요" 토스트 후 return, 아니면 `_dispatching=true`로 잠그고 fetch. `finally`에서 `_dispatching=false`로 해제(에러·네트워크 실패에도 반드시 풀림).

### 결과
- 성공: 버튼 더블탭·연타 시 같은 내용이 두 번 dispatch되던 것 방지. `node --check` 통과, 헤드리스 Chromium 검증 PAGE ERROR 0건·body 정상 렌더(childCount 5). 다른 로직 무변경.

### 배포 이력
- 버전: v2026.07.15.6 → v2026.07.15.7
- 배포 일시: 2026-07-23
