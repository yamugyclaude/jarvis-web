...
- 성공: Playwright(fetch 목킹) — 키없음(검색만)/AI성공(답변+참고기록)/AI배열에러(폴백+에러메시지 정상표시)/네트워크예외(폴백) 4가지 경로 모두 페이지 에러 0건으로 검증
...
- 성공: weekly.html Playwright 검증(fetch 목킹) — 목록 정렬/dir필터, 렌더링(h1/h2/li), <script> 태그 포함 악성 콘텐츠도 이스케이프되어 실행 안 됨(XSS 안전), 뒤로가기, 페이지 에러 0건
...
- 검증: 인라인 JS node --check + Playwright(fetch 목킹) — 인덱스 10건 빌드/캐시, "신혜식당" 3건, 문장형 "당구 언제 쳤어" OR확장 2건, 하이라이트, 없음 케이스, 페이지 에러 0건 모두 통과
